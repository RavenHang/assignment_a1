...
def run_multihead_self_attention(
    ...
    """
    from cs336_basics.model import CausalSelfAttention
    from cs336_basics.model import CausalSelfAttention
...
def run_transformer_block(
    ...
    block.load_state_dict(state_dict_mapped)
...
def run_transformer_lm(
    ...
    model = TransformerLM(
        ...
        dtype=in_indices.dtype if in_indices.is_floating_point() else torch.float32
    )
    
    state_dict_mapped = {}
    state_dict_mapped["token_embeddings.weight"] = weights["token_embeddings.weight"]
    state_dict_mapped["ln_final.weight"] = weights["ln_final.weight"]
    state_dict_mapped["lm_head.weight"] = weights["lm_head.weight"]
    
    for i in range(num_layers):
        state_dict_mapped[f"layers.{i}.attn_norm.weight"] = weights[f"layers.{i}.ln1.weight"]
        state_dict_mapped[f"layers.{i}.attn.w_q.weight"] = weights[f"layers.{i}.attn.q_proj.weight"]
        state_dict_mapped[f"layers.{i}.attn.w_k.weight"] = weights[f"layers.{i}.attn.k_proj.weight"]
        state_dict_mapped[f"layers.{i}.attn.w_v.weight"] = weights[f"layers.{i}.attn.v_proj.weight"]
        state_dict_mapped[f"layers.{i}.attn.w_o.weight"] = weights[f"layers.{i}.attn.output_proj.weight"]
        state_dict_mapped[f"layers.{i}.ffn_norm.weight"] = weights[f"layers.{i}.ln2.weight"]
        state_dict_mapped[f"layers.{i}.ffn.w1.weight"] = weights[f"layers.{i}.ffn.w1.weight"]
        state_dict_mapped[f"layers.{i}.ffn.w2.weight"] = weights[f"layers.{i}.ffn.w2.weight"]
        state_dict_mapped[f"layers.{i}.ffn.w3.weight"] = weights[f"layers.{i}.ffn.w3.weight"]

    model.load_state_dict(state_dict_mapped)
...
def run_rmsnorm(
    ...
    norm_layer.load_state_dict({"weight": weights})    
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    return clip_gradient_norm(parameters, max_l2_norm)
